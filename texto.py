...
Original file is located at
    https://colab.research.google.com/drive/1qVX51Q46r86_Z8QiqXQbdOjbMxL7zDXh
"""

import numpy as np

text_file = open("/EntradaPrimeiro.txt", "r")
arrlista = [x.strip().replace(" ", "") for x in text_file.readlines()]
    ...
    for j in i:
        if(j.isdigit() or j =='-' or j == '+'):
            ...
            if(aux != ''):
                ...
                if(aux == '+'):
                    aux = '+1'
                ArrAux.append(int(aux))
                aux = ''
    TotalAux1.append(ArrAux.copy())
    ArrAux.clear()
    TotalAux2.append(int(i.split('=',1)[1])) 

x = np.linalg.solve(TotalAux1, TotalAux2)

saidaTxt = open("/SaidaPrimeiro2.txt", "w")

count = 0
for i in x:
    if(count != len(x)-1):
        saidaTxt.write(str(ArrAux2[count]) + '=' + str(i) + ', ')
    else:
        saidaTxt.write(str(ArrAux2[count]) + '=' + str(i))
    count += 1
...
def Soma(arr, sum):
    if(len(arr) != 1):
        ...
        return 'NAO (Livros Na Caixa = 1)'
    
text_file = open("EntradaSegundo.txt", "r")
arrlista = [x.strip() for x in text_file.readlines()]
peso = int(arrlista[0])
k = int(arrlista[1])
arrlista.pop(0)
arrlista.pop(0)
...
for arr in arrlista:
    for i in arr.split():
        arrArray.append(int(i))
        
arrArray = np.array([arrArray[i:i + k] for i in range(0, len(arrArray), k)], dtype="object")

saidaTxt = open("SaidaSegundo.txt", "w")

contador = 1
for i in arrArray:
    saidaTxt.write("Caixa " + str(contador) + ": " + Soma(i, peso) + ' ' + str(i) + '\n')
    contador += 1
